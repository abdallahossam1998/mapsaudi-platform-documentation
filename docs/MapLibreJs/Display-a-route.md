...
Use the code sample below to display a route on your map with the MapLibre GL JS library using the [MapSaudi Routing API]().
...
<body>
  ...
  <script>
    ...
    map.on("load", function() {
      ...
      fetch(`https://api.jawg.io/routing/route/v1/car/${coordinates}?alternatives=false&geometries=geojson&overview=full&access-token=${accessToken}`)
